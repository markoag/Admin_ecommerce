...
import { CreateNestedVariationComponent } from '../create-nested-variation/create-nested-variation.component';
...
export class CreateVariationSpecificationsComponent {
  ...
  save() {
    ...
    this.attributeService.createSpecification(data).subscribe((resp: any) => {
      ...
      if (resp.message == 403) {
        ...
        this.toastr.success(
          'Exito',
          'Se registro la especificación correctamente'
        );
  ...
  editSpecification(specification: any) {
    const modal = this.modalService.open(EditVariationSpecificationsComponent, {
      centered: true,
      size: 'md',
    });
  ...
  openNested(variation: any){ 
    const modal = this.modalService.open(CreateNestedVariationComponent,{centered:true,size: 'lg'});
