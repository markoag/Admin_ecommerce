...
@Component({
  selector: 'app-edit-nested-variation',
  templateUrl: './edit-nested-variation.component.html',
  styleUrls: ['./edit-nested-variation.component.scss']
})
export class EditNestedVariationComponent {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      ...
      if (this.specification.attribute.type_attribute == 4) {
        ...
        this.selectedItems = this.specification.value_add
    ...
    if (this.is_variation) {
      this.precio_add = this.specification.price_add;
    ...
    this.attributeService
      ...
      .subscribe((resp: any) => {
        ...
        if (resp.message == 403) {
          ...
          this.toastr.success(
            'Exito',
            'Se actualizo la especificación correctamente'
          );
  ...
  storeVariation() {
    ...
    this.attributeService
      .updateVariationNested(this.specification.id, data)
      .subscribe((resp: any) => {
        console.log(resp);
        if (resp.message == 403) {
          this.toastr.error('Validación', resp.message_text);
          ...
          this.toastr.success(
            'Exito',
            'Se actualizo la variación anidada correctamente'
          );
          this.EspecificationE.emit(resp);
          this.modal.close();
        }
